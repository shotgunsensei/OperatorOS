...
| 16A | Re-baseline TradeFlowKit against the restored full product and close recorded parity gaps | Phase 15 release evidence plus restored-source provenance | In progress. The executable source ledger has zero unclassified items and 40 explicit gaps after nine real increments: Workflow Studio/team work management, persistent revenue-document mutations, bounded Directory-reconciled customer CSV import, full customer/job/task record editing, bounded tenant-scoped global search with canonical result navigation, tenant-safe archived customer/job/invoice listing plus dependency-ordered restore, replay-safe lead email/consent-gated SMS through the shared outbox, durable saved job views, and versioned bounded QuickBooks/Xero accounting exports. Destructive bulk deletion and permanent purge remain excluded under ADR-0011. No real standalone export, production apply, deployment, live-provider acceptance, or state-5 promotion is authorized. |
