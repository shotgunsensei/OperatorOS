...
export default function CallCommandCommercialWorkspace({ view, recordId, hrefFor }: { view: CallCommandRouteArea; recordId?: string; hrefFor: (path: string) => string }) {
  ...
  const telephonyProvider = product?.providers?.telephony as Row | undefined;
  const telephonyReady = telephonyProvider?.configured === true
    || telephonyProvider?.ready === true
    || Boolean(telephonyProvider?.fromNumber);
  ...
  async function simulate() {
    ...
    // delayed route transition to the call-detail URL.
    if (callId) setSelectedCallId(String(callId));
  }
  ...
  return <section data-testid={`callcommand-commercial-${view}`} style={{ minWidth: 0, color: semantic.text }}>
    ...
    {view === 'actions' && <FollowUpWorkspace product={commercialProduct} canWrite={canWrite} busy={busy} onAdvance={(type, item) => void run(`work:${item.id}`, () => moduleShellApi.callcommand.productUpdateObject(type, item.id, { status: type === 'leads' ? 'contacted' : 'completed' }), `${type === 'leads' ? 'Lead' : type === 'tickets' ? 'Ticket' : 'Task'} updated.`)}/>}
...
function FollowUpWorkspace({ product, canWrite, busy, onAdvance }: { product: ProductWorkspace | null; canWrite: boolean; busy: string; onAdvance: (type: 'tickets' | 'leads' | 'tasks', item: Row) => void }) {
  const groups: Array<{ type: 'tickets' | 'leads' | 'tasks'; title: string; items: Row[] }> = [
    { type: 'tickets', title: 'Tickets', items: product?.tickets ?? [] },
    { type: 'leads', title: 'Leads', items: product?.leads ?? [] },
    { type: 'tasks', title: 'Tasks', items: product?.tasks ?? [] },
  ];
    ...
    {groups.map(group => <section key={group.type} style={panel}>
      ...
      {group.items.length ? <div style={{ display: 'grid', gap: 8 }}>{group.items.slice(0, 20).map(item => {
        ...
        return <article key={item.id} style={{ padding: 10, border: '1px solid rgba(94,234,212,.14)', borderRadius: 9 }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', justifyContent: 'space-between' }}><strong>{item.title ?? item.name ?? item.company ?? 'Call follow-up'}</strong><Badge tone={terminal ? 'good' : 'neutral'}>{item.status}</Badge></div>
          {!terminal && <button style={{ ...quiet, marginTop: 8, ...disabledStyle(canWrite && !busy) }} disabled={!canWrite || !!busy} onClick={() => onAdvance(group.type, item)}>Advance</button>}
        </article>;
      })}</div> : <span style={{ color: semantic.textMuted }}>No {group.title.toLowerCase()} have been generated.</span>}
    </section>)}
  </section>;
}
