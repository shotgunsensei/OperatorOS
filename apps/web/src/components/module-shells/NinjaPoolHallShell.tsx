...
export default function NinjaPoolHallShell({ routePath, embedded = false, gameActive = false }: { baseUrl?: string; routePath?: string; embedded?: boolean; gameActive?: boolean }) {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setError(cause?.error || cause?.message || 'Unable to load Ninja Pool Hall profile.');
    }
  }, []);
  ...
  return (
    <div id="ninja-pool-hall-shell" className="nph-shell" data-testid="ninja-pool-hall-shell">
      ...
      <div className="nph-shell-wrap">
        ...
        {!profileData ? (
          <div className="nph-shell-loading"><Loader2 className="spin" size={24} /> Loading your table…</div>
        ) : view === 'home' ? (
          <section className="nph-home" data-testid="ninja-pool-dashboard">
            <div className="nph-home-hero">
              ...
              <div className="nph-home-stats">
                ...
                <span><b>{profileData.progression.localMatches}</b> hot-seat results</span>
              </div>
            </div>
          ...
          <NinjaPoolHallProfile
            ...
            onSaved={(profile) => setProfileData((current) => current ? { ...current, profile } : current)}
          />
        ) : matchId ? (
          <NinjaPoolHallMatchDetail matchId={matchId} onBack={() => navigate('home')} />
        ) : (
          <button type="button" onClick={() => navigate('home')}>Return to hall</button>
        )}

        {view === 'home' && profileData && profileData.progression.matchesCompleted > 0 && (
          <button className="nph-hidden-detail-link" type="button" onClick={() => {
