    ...
    async (request, reply) => {
      try {
        ...
        UPDATE torqueshed_vehicles SET
          ...
          year=CASE WHEN ${'year' in body} THEN ${torqueInteger(body.year, 'year', 1886, new Date().getFullYear() + 2, true)} ELSE year END,
          make=CASE WHEN ${'make' in body} THEN ${torqueText(body.make, 'make', 100, { required: true, min: 1, singleLine: true })} ELSE make END,
          model=CASE WHEN ${'model' in body} THEN ${torqueText(body.model, 'model', 100, { required: true, min: 1, singleLine: true })} ELSE model END,
  ...
  app.patch(
    ...
    async (request, reply) => {
      try {
        ...
        const version = expectedVersion(body);
        const result = await db.execute(
          sql`UPDATE torqueshed_service_records r SET title=CASE WHEN ${'title' in body} THEN ${torqueText(body.title, 'title', 180, { required: true, min: 2, singleLine: true })} ELSE title END,description=CASE WHEN ${'description' in body} THEN ${torqueText(body.description, 'description', 10000)} ELSE description END,status=CASE WHEN ${'status' in body} THEN ${torqueEnum(body.status, 'status', ['planned', 'in_progress', 'completed', 'canceled'] as const, 'completed')} ELSE status END,labor_minutes=CASE WHEN ${'laborMinutes' in body} THEN ${torqueInteger(body.laborMinutes, 'laborMinutes', 0, 1_000_000)} ELSE labor_minutes END,labor_cost_minor=CASE WHEN ${'laborCostMinor' in body} THEN ${torqueInteger(body.laborCostMinor, 'laborCostMinor', 0, 2_147_483_647)} ELSE labor_cost_minor END,parts_cost_minor=CASE WHEN ${'partsCostMinor' in body} THEN ${torqueInteger(body.partsCostMinor, 'partsCostMinor', 0, 2_147_483_647)} ELSE parts_cost_minor END,other_cost_minor=CASE WHEN ${'otherCostMinor' in body} THEN ${torqueInteger(body.otherCostMinor, 'otherCostMinor', 0, 2_147_483_647)} ELSE other_cost_minor END,version=version+1,updated_by_user_id=${user(request)},updated_at=NOW() FROM torqueshed_vehicles v WHERE r.tenant_id=${tenant(request)} AND r.id=${id} AND r.version=${version} AND r.archived_at IS NULL AND v.tenant_id=r.tenant_id AND v.id=r.vehicle_id AND v.archived_at IS NULL AND (${canManage(request)} OR v.owner_user_id=${user(request)}) RETURNING r.*`,
        );
        const updated = first(result);
        if (!updated)
          return reply.code(409).send({
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        const row = first(
          await db.execute(
            sql`INSERT INTO torqueshed_diagnostic_templates (tenant_id,owner_user_id,name,description,concern_pattern,test_plan,visibility,created_by_user_id,updated_by_user_id) VALUES (${tenant(request)},${user(request)},${torqueText(b.name, 'name', 180, { required: true, min: 2, singleLine: true })},${torqueText(b.description, 'description', 5000)},${torqueText(b.concernPattern, 'concernPattern', 5000)},${jsonArray(b.testPlan, 'testPlan', 100)},${torqueEnum(b.visibility, 'visibility', ['private', 'tenant'] as const, 'private')},${user(request)},${user(request)}) RETURNING *`,
          ),
        )!;
  ...
  try {
    ...
    const query =
      table === 'torqueshed_build_tasks'
        ? sql`UPDATE torqueshed_build_tasks c SET title=COALESCE(${title},title),notes=CASE WHEN ${'notes' in b} THEN ${torqueText(b.notes, 'notes', 5000)} ELSE notes END,status=COALESCE(${status},status),due_at=CASE WHEN ${'dueAt' in b} THEN ${torqueDate(b.dueAt, 'dueAt')} ELSE due_at END,completed_at=CASE WHEN ${status === 'completed'} THEN NOW() WHEN ${status !== null} THEN NULL ELSE completed_at END,version=version+1,updated_by_user_id=${user(request)},updated_at=NOW() FROM torqueshed_builds p WHERE c.tenant_id=${tenant(request)} AND c.id=${id} AND c.version=${version} AND c.archived_at IS NULL AND p.tenant_id=c.tenant_id AND p.id=c.build_id AND (${canManage(request)} OR p.owner_user_id=${user(request)}) RETURNING c.*`
        : sql`UPDATE torqueshed_build_stages c SET title=COALESCE(${title},title),description=CASE WHEN ${'description' in b} THEN ${torqueText(b.description, 'description', 5000)} ELSE description END,status=COALESCE(${status},status),version=version+1,updated_by_user_id=${user(request)},updated_at=NOW() FROM torqueshed_builds p WHERE c.tenant_id=${tenant(request)} AND c.id=${id} AND c.version=${version} AND c.archived_at IS NULL AND p.tenant_id=c.tenant_id AND p.id=c.build_id AND (${canManage(request)} OR p.owner_user_id=${user(request)}) RETURNING c.*`;
...
async function updateReminder(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const version = expectedVersion(b);
    const result = await db.execute(
      sql`UPDATE torqueshed_service_reminders r SET title=CASE WHEN ${'title' in b} THEN ${torqueText(b.title, 'title', 180, { required: true, min: 2, singleLine: true })} ELSE title END,notes=CASE WHEN ${'notes' in b} THEN ${torqueText(b.notes, 'notes', 5000)} ELSE notes END,due_at=CASE WHEN ${'dueAt' in b} THEN ${torqueDate(b.dueAt, 'dueAt')} ELSE due_at END,due_mileage=CASE WHEN ${'dueMileage' in b} THEN ${torqueInteger(b.dueMileage, 'dueMileage', 0, 10_000_000)} ELSE due_mileage END,status=CASE WHEN ${'status' in b} THEN ${torqueEnum(b.status, 'status', TORQUESHED_REMINDER_STATUSES, 'open')} ELSE status END,version=version+1,updated_by_user_id=${user(request)},updated_at=NOW() FROM torqueshed_vehicles v WHERE r.tenant_id=${tenant(request)} AND r.id=${id} AND r.version=${version} AND r.archived_at IS NULL AND v.tenant_id=r.tenant_id AND v.id=r.vehicle_id AND (${canManage(request)} OR v.owner_user_id=${user(request)}) RETURNING r.*`,
    );
    const updated = first(result);
    if (!updated)
      return reply
...
async function updateDiagnosticEntry(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const version = expectedVersion(b);
    const result = await db.execute(
      sql`UPDATE torqueshed_diagnostic_entries e SET title=CASE WHEN ${'title' in b} THEN ${torqueText(b.title, 'title', 180, { required: true, min: 1, singleLine: true })} ELSE title END,value_text=CASE WHEN ${'valueText' in b} THEN ${torqueText(b.valueText, 'valueText', 10000)} ELSE value_text END,value_numeric=CASE WHEN ${'valueNumeric' in b} THEN ${torqueNumber(b.valueNumeric, 'valueNumeric', -1e15, 1e15)} ELSE value_numeric END,unit=CASE WHEN ${'unit' in b} THEN ${torqueText(b.unit, 'unit', 40, { singleLine: true })} ELSE unit END,reference_min=CASE WHEN ${'referenceMin' in b} THEN ${torqueNumber(b.referenceMin, 'referenceMin', -1e15, 1e15)} ELSE reference_min END,reference_max=CASE WHEN ${'referenceMax' in b} THEN ${torqueNumber(b.referenceMax, 'referenceMax', -1e15, 1e15)} ELSE reference_max END,outcome=CASE WHEN ${'outcome' in b} THEN ${torqueText(b.outcome, 'outcome', 5000)} ELSE outcome END,version=version+1,updated_by_user_id=${user(request)},updated_at=NOW() FROM torqueshed_diagnostic_sessions d WHERE e.tenant_id=${tenant(request)} AND e.id=${id} AND e.version=${version} AND e.archived_at IS NULL AND d.tenant_id=e.tenant_id AND d.id=e.diagnostic_session_id AND (${canManage(request)} OR d.owner_user_id=${user(request)}) RETURNING e.*`,
    );
    const updated = first(result);
    if (!updated)
      return reply.code(409).send({
...
async function updateTemplate(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const version = expectedVersion(b);
    const result = await db.execute(
      sql`UPDATE torqueshed_diagnostic_templates SET name=CASE WHEN ${'name' in b} THEN ${torqueText(b.name, 'name', 180, { required: true, min: 2, singleLine: true })} ELSE name END,description=CASE WHEN ${'description' in b} THEN ${torqueText(b.description, 'description', 5000)} ELSE description END,concern_pattern=CASE WHEN ${'concernPattern' in b} THEN ${torqueText(b.concernPattern, 'concernPattern', 5000)} ELSE concern_pattern END,test_plan=CASE WHEN ${'testPlan' in b} THEN ${jsonArray(b.testPlan, 'testPlan', 100)} ELSE test_plan END,visibility=CASE WHEN ${'visibility' in b} THEN ${torqueEnum(b.visibility, 'visibility', ['private', 'tenant'] as const, 'private')} ELSE visibility END,version=version+1,updated_by_user_id=${user(request)},updated_at=NOW() WHERE tenant_id=${tenant(request)} AND id=${id} AND version=${version} AND archived_at IS NULL AND (${canManage(request)} OR owner_user_id=${user(request)}) RETURNING *`,
    );
    const updated = first(result);
    if (!updated)
      return reply.code(409).send({
