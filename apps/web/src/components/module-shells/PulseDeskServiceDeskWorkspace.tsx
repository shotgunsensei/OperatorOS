...
export default function PulseDeskServiceDeskWorkspace({ tenantKey, canManageModule }: { tenantKey: string; canManageModule: boolean }) {
  ...
  useEffect(() => {
    ...
    else if (/\/(service-desk\/admin|service-desk-admin|settings)(\/|$)/.test(path)) setTab('admin');
    else if (/\/analytics(\/|$)/.test(path)) setTab('dashboard');
  }, [loadTicket]);
