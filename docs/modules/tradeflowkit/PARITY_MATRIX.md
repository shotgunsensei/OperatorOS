...
first Workflow Studio increment, it classifies 96 items active, 53 as shared
OperatorOS replacements, 66 as open Phase 16 gaps, 39 as retired for security,
...
The 66 open items are not waived by the earlier approved-scope state 4
...
| Quotes CRUD, public view, acceptance, rejection, expiration | revenue flow; public quote page/API; controlled response | numbered/versioned quotes + normalized quote items + token hash | public decision and conversion test | partial / Phase 16 gap: standalone-equivalent edit/archive is open |
| Quote-to-invoice and quote-to-job | quote-to-invoice is idempotent; lead conversion creates jobs | unique source quote + transaction | duplicate retry test | partial / Phase 16 gap: direct quote-to-job conversion is open |
| Invoices CRUD/public view/payment link/email | revenue flow; public invoice page; public-link and messaging APIs | numbered/versioned invoices + normalized items + token hash + shared outbox | workflow + public projection | partial / Phase 16 gap: direct create/edit/archive/import/bulk behavior remains open; provider processing remains gated |
...
contains 66 explicit gaps. Workflow Studio, team job-task views, job
transitions, activity, schema readiness, and focused persistence/isolation
coverage are now real active increments. Phase 16A also adds a read-only
...
has moved. These increments do not erase invoice,
provider, import/bulk, scheduling, saved-view, public-intake, retention,
