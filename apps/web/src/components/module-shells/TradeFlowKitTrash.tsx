...
export default function TradeFlowKitTrash({
  ...
  const load = useCallback(async () => {
    ...
    try { setData(await moduleShellApi.tradeflowkit.trash()); }
    ...
    <section className="tfk-trash" id="tradeflowkit-trash" tabIndex={-1} data-testid="tradeflowkit-trash">
      ...
      {loading ? (
        ...
        <div className="tfk-trash-groups" data-testid="tradeflowkit-trash-groups">
          <TrashGroup title="Customers" kind="customers" Icon={Users} rows={data.customers} hasMore={data.hasMore.customers} canManage={canManage} restoring={restoring} onRestore={restore} render={row => {
          ...
          <TrashGroup title="Jobs" kind="jobs" Icon={BriefcaseBusiness} rows={data.jobs} hasMore={data.hasMore.jobs} canManage={canManage} restoring={restoring} onRestore={restore} render={row => {
          ...
          <TrashGroup title="Invoices" kind="invoices" Icon={Receipt} rows={data.invoices} hasMore={data.hasMore.invoices} canManage={canManage} restoring={restoring} onRestore={restore} render={row => {
...
function TrashGroup({
  title, kind, Icon, rows, hasMore, canManage, restoring, onRestore, render,
}: {
