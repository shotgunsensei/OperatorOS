...
export default function PulseDeskServiceDeskWorkspace({ tenantKey, canManageModule }: { tenantKey: string; canManageModule: boolean }) {
  ...
  if (loading && !dashboard) return <div className="pds-loading" data-testid="pulsedesk-service-desk-loading"><RefreshCw className="pds-spin" size={18} /> Loading your operations workspace…<style>{css}</style></div>;
  ...
  return (
    <section className="pds" data-testid="pulsedesk-service-desk-workspace">
      ...
      {tab === 'admin' && <div className="pds-two">
        ...
        {canManageModule ? <section className="pds-stack"><form className="pds-card pds-form" onSubmit={createQueue}><Heading icon={UsersRound} title="Add queue" subtitle="Route incoming operational work to the right team." /><input name="name" required placeholder="Queue name" /><textarea name="description" placeholder="Queue purpose" /><button>Add queue</button></form><form className="pds-card pds-form" onSubmit={createSla}><Heading icon={Clock3} title="Add SLA policy" subtitle="Set elapsed-time response and resolution targets for your teams." /><input name="name" required placeholder="Policy name" /><div className="pds-grid-3"><input name="responseMinutes" type="number" min={1} defaultValue={60} /><input name="resolutionMinutes" type="number" min={1} defaultValue={480} /><input name="atRiskPercent" type="number" min={1} max={99} defaultValue={80} /></div><label className="pds-check"><input type="checkbox" name="defaultPolicy" /> Workspace default</label><button>Add policy</button></form></section> : <section className="pds-card"><Empty text="Ask a workspace manager for access to these settings." /></section>}
      </div>}
    </section>
  );
}
...
function Dashboard({ dashboard, tickets, onOpen }: { dashboard: PulseDeskServiceDashboard; tickets: PulseDeskServiceTicket[]; onOpen: (id: string) => void }) {
  ...
  return <div className="pds-stack"><div className="pds-metrics">{metrics.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div><section className="pds-card"><Heading icon={Activity} title="Current operational work" subtitle={`Updated ${formatDate(dashboard.generatedAt)}.`} />{tickets.slice(0, 8).map(ticket => <button className="pds-row-button" key={ticket.id} onClick={() => onOpen(ticket.id)}><span><strong>{ticket.humanId}</strong> {ticket.summary}</span><small>{ticket.status} · {ticket.priority} · SLA {ticket.sla.state}</small></button>)}</section></div>;
}
