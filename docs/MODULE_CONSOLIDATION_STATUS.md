...
| --- | --- | --- | --- | --- | --- |
| TradeFlowKit | `tradeflowkit` | `tradeflowkit.operatoros.net` | core | Original baseline `6d0c13df5e324f6aba9cdf2cf14a550d0cf0ca55`; clean restored product evidence `C:\Dev\TradeFlowKit` at `37aa67f1da804fc3ac56f36e50e01362077d7a26` | Approved-scope state 4 remains, but full Phase 16 parity is open: the zero-unclassified source ledger records 115 active capabilities and 45 explicit gaps. Existing revenue/field-service workflows now include Workflow Studio, governed job transitions, team job-task views, activity, direct invoice creation, versioned quote/invoice draft editing, history-safe archive/restore, idempotent quote-to-job, source-valid high priority, bounded Directory-reconciled customer CSV import, tenant-scoped global search, durable personal/admin-shared job views, bounded archived-record restore, and replay-safe lead email/consent-gated SMS through the shared outbox. Destructive bulk deletion and permanent purge remain retired by ADR-0011. A read-only snapshot and guarded atomic v1 core-data apply pass synthetic isolated rehearsal; no real export/production cutover occurred and later import versions are required. Release v30 and the exact-host saved-view workflow pass locally; deployed provider and authenticated acceptance still block state 5 |
...
machine-readable ledger has zero unclassified source items and 49 explicit
...
tenant-scoped global search spans eight active record groups with bounded,
escaped queries and canonical module-host result navigation. The new
retention workspace lists archived customers, jobs, and invoices through safe
tenant-scoped projections and restores them with optimistic versions,
dependency checks, advisory locks, and activity; permanent purge remains
absent. Lead email and consent-gated SMS use server-owned destinations plus
the shared idempotent outbox; no module-local provider credentials or delivery
claims were added. The new
...
3. Deploy and verify the reviewed Ninja Pool Hall Phase 10B and TorqueShed
   State 4 revisions before state 5; keep Ninja Pool Hall's online relay
