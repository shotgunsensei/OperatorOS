...
| TradeFlowKit | Source/local state 4 candidate: lead conversion, replay-safe shared-outbox email/consent-gated SMS, versioned lead settings/templates, transactional manually actioned follow-ups, and sanitized adapter validation/history; full customer → job/work order → task single-record CRUD/deep links; shared Directory customers and bounded CSV import; tenant-scoped global search; durable personal/admin-shared job views; format-v1 QuickBooks IIF/invoice CSV and Xero customer/invoice/payment CSV; bounded archived customer/job/invoice restore; admin-only bounded job status and job/invoice restore batches; exact-balance invoice settlement batches; numbered jobs/tasks/dependencies; Workflow Studio; direct and quote-derived invoices; versioned multi-line quote/invoice editing; history-safe archive; idempotent quote-to-job/invoice; payments; portal/documents; settings; analytics | Server guards, server-owned lead and admin-test destinations, SMS consent/opt-out, exact replay/body-drift rejection, safe adapter projections, optimistic settings/follow-ups, composite tenant/lead FKs, stable row locking, all-or-nothing optimistic batches, dependency-ordered archive/restore, exact first-class payment rows, bounded escaped search, saved-view input/ownership/share controls, bounded accounting projections, formula neutralization, successful-payment-ledger selection, persistence/restart, viewer denial, safe projections, Directory preservation, and cross-tenant tests pass | Pass | Lead-operations static and isolated workflows 6/6; adjacent lead/bulk/retention/revenue/saved-view/work-management regression 27/27; final clean aggregate 900 pass, 0 fail, 6 HTTP-only skips across 906 tests | Production build and readiness-gated 31-step compiled runtime pass locally at database release v31/31; deployment not run | Updated exact-host workflow passes 1/1 in 21.8 seconds and exercises PKCE, template apply, adapter validation, transactional follow-up scheduling/queue, admin delivery check, then the existing persisted customer/job/task workflow. No provider delivery or deployed workflow/cutover run | **Not production-ready** |
...
135 active items, 53 shared replacements, 23 explicit gaps, zero unclassified,
43 security retirements, and 23 product-boundary retirements. The latest
increment adds versioned internal lead settings/capture profile/templates,
transactional manually actioned follow-ups, sanitized adapter validation and
history, and authenticated-admin delivery checks while ADR-0030 keeps
automatic response, module-local provider credentials, and anonymous intake
off. Focused lead-operations checks pass 6/6, adjacent regression passes 27/27,
the final clean aggregate passes 900/0/6 across 906 tests, and v31 compiled
readiness plus the authenticated exact-host browser gate pass locally.
Deployment, providers, real data apply/cutover, rollback rehearsal, and all 23
