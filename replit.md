...
- **Pluggable Push Adapters (Task #109):** `apps/api/src/lib/entitlement-adapters.ts` defines `EntitlementPushAdapter` and selects per-module wire shape from the `modules` columns `push_shape`, `push_auth_mode`, `push_bearer_env_var`. Default is `canonical_snapshot` (HMAC, one push per member×receiver). TradeFlowKit ships as `tradeflowkit_v1` (bearer-token, one batched push per receiver, 12-key feature whitelist). The adapter is presentation-only — the resolver snapshot remains the single source of truth. Legacy alias `POST /modules/sso/consume` (no `/v1`) is also mounted for TradeFlowKit's hard-coded path. See `docs/MODULE_SSO.md` §3.6 / §3.7.
