...
| TradeFlowKit | Source/local state 4 candidate: lead conversion; full customer → job/work order → task single-record CRUD/deep links; shared Directory customers and bounded CSV import; tenant-scoped global search across eight active record groups; bounded archived customer/job/invoice listing and restore; numbered jobs/tasks/dependencies; Workflow Studio; direct and quote-derived invoices; versioned multi-line quote/invoice editing; history-safe archive; idempotent quote-to-job/invoice; payments; portal/documents; messaging; settings; analytics; CSV export | Server guards, optimistic versions, dependency-ordered archive/restore, exact-key import replay/body-drift rejection, bounded escaped search, persistence/restart, viewer denial, safe retention projections, normalized duplicate/item reconciliation, Directory mapping/preservation, and cross-tenant tests pass | Pass | Fresh retention/search adjacent PostgreSQL regression: 22/22; retention static/routing: 4/4; prior clean aggregate: 866 pass, 0 fail, 6 HTTP-only skips | Production build and readiness-gated 29-step compiled runtime pass locally; deployed target is an older revision | Exact-host archive/restore workflow passes 1/1 in 19.3 seconds with PKCE return, customer/job/task create-edit-deep-link, refresh, My Apps return/reopen, safe archive order, canonical `/trash`, dependency-ordered customer/job restore, and preserved task archive; global-search and customer-import flows separately pass 1/1. Deployed workflow/cutover not run | **Not production-ready** |
...
109 active items, 53 shared replacements, 51 explicit gaps, zero unclassified,
...
data apply/cutover, rollback rehearsal, and all 51 remaining ledger gaps keep
