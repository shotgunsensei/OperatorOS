...
Assessment updated: 2026-07-22. Scope: OperatorOS and consolidated modules in
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| OperatorOS | Identity, tenant, entitlement, billing, audit, launcher | Local SSO/RBAC/tenant tests pass | Pass | Pass | Local pass; deployed target not run | Local production-host matrix 4/4; deployed target not run | **Not production-ready** |
...
| TorqueShed | Phase 9 source candidate: Phase 7 automotive foundation; safety-ranked Assist and append-only token accounting; persistent Marketplace/Community, scanned media, blocks, reports and append-only moderation with native UI | Phase 9 domain/static contracts pass; the current aggregate reproduces pre-existing registry/lifecycle failures in foundation and Assist | Pass | PostgreSQL and the 21-step release pass; Marketplace/Community workflow passes, but foundation/Assist aggregate rows are not green | Production build passes; compiled runtime and deployed target not run | Existing shared SSO/shell evidence predates Phase 7-9; new `/diagnostics`, Assist, Marketplace and Community browser rerun remains open | **Not production-ready** |
...
| Ninja Pool Hall | Phase 10B source/local state-4 candidate: Free Shoot, CPU 8-ball, local hot-seat, profiles/preferences, structured rules/results/events, aggregates, recovery and persistent deep links | Trusted session scope, versions, idempotency, viewer denial, cross-tenant/user 404s, bounded shot facts, append-only events and explicit client-reported trust label pass focused tests | Pass | Scoped workflows and clean/idempotent 22-step release pass; full aggregate retains two unrelated TorqueShed failures | Compiled 22-step runtime and canonical HTTPS health/readiness pass locally; deployed target not run | Production-host matrix 4/4 covers real canvas CPU/local shots, persistence, result deep refresh, return, global logout, reauthentication and mobile navigation; deployed workflow not run | **Not production-ready** |
