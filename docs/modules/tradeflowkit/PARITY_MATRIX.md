...
retention, lead-messaging, saved-view, accounting-export, and safe-bulk increments, it classifies 124 items active, 53 as shared OperatorOS replacements, 34 as open
Phase 16 gaps, 43 as retired for security,
...
The 34 open items are not waived by the earlier approved-scope state 4
...
| Jobs CRUD, schedule, assignment, events | operations board with full editor/deep link; `POST /jobs`, `GET/PATCH/DELETE /jobs/:id`; admin-only `POST /jobs/bulk-status`; archived-record restore workspace | numbered/versioned `tradeflowkit_jobs`, bounded all-or-nothing status batch, dependency-guarded soft archive/restore, activity | core CRUD, safe-bulk, and retention exact-host/API restart workflows | complete |
...
| Invoices CRUD/public view/payment link/email | revenue flow with direct create, multi-line draft editor, history-safe archive/restore; public invoice page; public-link and messaging APIs | numbered/versioned invoices + transactionally reconciled normalized items + token hash + shared outbox | document-mutation/retention workflow + public projection | partial / Phase 16 gap: invoice import remains open; provider processing remains gated |
