...
export default function PulseDeskServiceDeskWorkspace({ tenantKey, canWriteModule, canManageModule, view, requestHref, routeHref }: { tenantKey: string; canWriteModule: boolean; canManageModule: boolean; view: PulseDeskServiceView; requestHref: (id: string) => string; routeHref: (path: string) => string }) {
  ...
  async function mutate(label: string, operation: () => Promise<unknown>, options: { detailId?: string; reset?: HTMLFormElement } = {}) {
    if (!canWriteModule) {
      setError('This access level can review PulseDesk records but cannot change them.');
      return;
    }
    ...
    <section className="pds" data-testid="pulsedesk-service-desk-workspace">
      ...
      {view === 'tickets' && <div className="pds-ticket-layout">
        <div className="pds-stack">
          ...
          </form> : <section className="pds-card"><Empty text="Ask a PulseDesk user or manager to create an operational ticket." /></section>}
...
function Dashboard({ dashboard, tickets, requestHref, routeHref, canManage }: { dashboard: PulseDeskServiceDashboard; tickets: PulseDeskServiceTicket[]; requestHref: (id: string) => string; routeHref: (path: string) => string; canManage: boolean }) {
  ...
  return <div className="pds-stack">
    ...
    {trainingTicket && <OutcomeWorkflowAction
      ...
      sourceDeepLink={`/modules/pulsedesk/tickets/${trainingTicket.id}`}
      sourceModuleSlug="pulsedesk"
      sourceType="pulsedesk_request"
      sourceKind="ticket"
      payload={{ authorApproved: true, privacyReviewed: true }}
      title="Turn a resolved operations issue into team practice"
      description={`Reuse the lesson from ${trainingTicket.humanId} without copying the request into a separate training tool.`}
      destinationLabel="FaultlineLab"
      actionLabel="Preview training exercise"
      confirmationText="Create an unpublished training draft with common identifiers masked. I have reviewed this operations request and confirm it contains no patient or clinical information; a trainer must complete a full privacy and accuracy review before publication."
      disabled={!canManage}
      disabledReason={!canManage ? 'A PulseDesk workspace manager must approve this training draft.' : undefined}
      previewItems={[
        { label: 'Operations training scenario', detail: 'The request and resolution become a draft learner briefing' },
...
function TicketDetail({ detail, busy, attachments, configuration, assignees, vendors, canWrite, canManage, onUpdate, onReply, onTime, onAssign, onTransition, onEvaluate, onUpload, onVendor }: {
  ...
  return <aside className="pds-card pds-detail" data-testid="pulsedesk-ticket-workspace"><Heading icon={Inbox} title={`${ticket.humanId} · ${ticket.summary}`} subtitle={`${ticket.status.replaceAll('_', ' ')} · ${ticket.priority} · SLA ${ticket.sla.state.replaceAll('_', ' ')}`} /><p className="pds-description">{ticket.description || 'No additional operational description.'}</p><div className="pds-chips"><span>Created {formatDate(ticket.createdAt)}</span><span>Response due {formatDate(ticket.responseDueAt)}</span><span>Resolution due {formatDate(ticket.resolutionDueAt)}</span></div>
    ...
    {canWrite && <form className="pds-form" onSubmit={event => onReply(event, 'requester')}><textarea name="body" required aria-label="Requester-visible operational reply" placeholder="Requester-visible operational reply" /><label className="pds-ack">{PHI_WARNING}</label><button type="submit">Add requester-visible reply</button></form>}
    {canWrite && detail.capabilities.canViewInternal && <><form className="pds-form" onSubmit={event => onReply(event, 'internal')}><textarea name="body" required aria-label="Internal service-team note" placeholder="Internal service-team note" /><label className="pds-ack">Internal notes are never returned to requester/viewer roles. {PHI_WARNING}</label><button type="submit">Add internal note</button></form><form className="pds-inline" onSubmit={onTime}><input name="minutes" aria-label="Minutes worked" type="number" min={1} max={1440} required placeholder="Minutes" /><select name="workType" aria-label="Work type"><option>onsite</option><option>remote</option><option>vendor</option><option>administrative</option></select><input name="description" aria-label="Operational work summary" placeholder="Operational work summary" /><button type="submit">Log time</button></form></>}
