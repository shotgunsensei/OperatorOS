...
export async function ensureTradeFlowKitTables(): Promise<void> {
  await db.execute(`
    ...
    CREATE TABLE IF NOT EXISTS tradeflowkit_saved_views (
      ...
      sort JSONB NOT NULL DEFAULT '{"field":"updatedAt","direction":"desc"}'::jsonb,
      ...
      updated_at TIMESTAMP DEFAULT NOW() NOT NULL,
      archived_at TIMESTAMP,
      CONSTRAINT tfk_saved_views_resource_check CHECK (resource IN ('search','leads','customers','jobs','tasks','quotes','invoices')),
      CONSTRAINT tfk_saved_views_name_check CHECK (char_length(name) BETWEEN 1 AND 120),
      CONSTRAINT tfk_saved_views_filters_check CHECK (jsonb_typeof(filters) = 'object'),
      CONSTRAINT tfk_saved_views_sort_check CHECK (jsonb_typeof(sort) = 'object'),
      CONSTRAINT tfk_saved_views_version_check CHECK (version >= 1)
    );
    CREATE UNIQUE INDEX IF NOT EXISTS uq_tfk_saved_views_active_name
      ON tradeflowkit_saved_views(tenant_id, user_id, resource, name)
      WHERE archived_at IS NULL;
    CREATE INDEX IF NOT EXISTS idx_tfk_saved_views_tenant_resource
      ON tradeflowkit_saved_views(tenant_id, resource, is_shared, archived_at);
