...
test('database release plan is explicit, ordered, additive, and reusable by startup', async () => {
  ...
  assert.equal(release.DATABASE_RELEASE_CONTRACT.releaseVersion, 51);
  ...
  assert.equal(release.DATABASE_RELEASE_STEPS.length, 51);
  assert.equal(new Set(release.DATABASE_RELEASE_STEPS.map((step: { id: string }) => step.id)).size, 51);
  assert.equal(release.DATABASE_RELEASE_STEPS[0].id, 'base_tables');
  assert.equal(release.DATABASE_RELEASE_STEPS.at(-1).id, 'torqueshed_settlement_contract');
  assert.ok(
