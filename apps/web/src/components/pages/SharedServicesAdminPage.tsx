...
import { serviceStateLabel } from '@/lib/service-readiness';
...
function StateBadge({ value }: { value: string }) {
  ...
  return <span style={{ color, border: `1px solid ${color}66`, borderRadius: 999, padding: '2px 7px', fontSize: 11 }}>{serviceStateLabel(value)}</span>;
}
...
export default function SharedServicesAdminPage({ onNavigate }: Props) {
  ...
  return <SharedServicesWorkspace key={tenantId} tenantId={tenantId} onNavigate={onNavigate} />;
}

function SharedServicesWorkspace({ tenantId, onNavigate }: Props & { tenantId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (version === requestVersion.current) {
        setOverview(null); setOperations(null); setModules([]); setEndpoints([]); setFabricRuns([]);
        setError('We could not load your connections. Try again. Your saved settings have not changed.');
      }
  ...
  const run = async (key: string, action: () => Promise<unknown>) => {
    ...
    catch { setError('This change could not be confirmed. Refresh and check the saved result before trying again.'); }
    finally { setBusy(null); }
  };

  return (
    <div style={{ padding: 'clamp(14px, 3vw, 32px)', maxWidth: 1180, margin: '0 auto', color: semantic.text, overflowWrap: 'anywhere' }} data-testid="page-shared-services-admin">
      <header style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'center', marginBottom: space.xl }}>
        ...
        <button data-testid="button-refresh-shared-services" style={buttonStyles.secondary} disabled={loading || !!busy} onClick={() => void refresh()}><RefreshCw size={14} /> {loading ? 'Checking…' : 'Refresh'}</button>
      </header>
      ...
      {loading && <p role="status">Loading your connections and activity…</p>}
      {!loading && overview && <>
      <ServiceReadiness providers={overview.runtimeProviders ?? []} attachments={overview.attachments} />
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 12, marginBottom: space.lg }} aria-label="Shared service health">
        {[
          ['Background work', overview?.worker?.started ? 'ready' : 'blocked', 'Completes scheduled work and retries supported tasks'],
          ['Secure connections', overview?.secretVault?.configured ? 'ready' : overview?.secretVault?.mode === 'test' ? 'test' : 'blocked', 'Protects saved service credentials'],
          ...
          ['Access keys', String(overview?.counts?.active_api_tokens ?? 0), 'active connections with limited permissions'],
          ['Credits used', String(overview?.counts?.credits_consumed ?? 0), 'recorded usage'],
      ...
      <section style={{ ...cardStyle, marginBottom: space.lg }} data-testid="shared-team-access-links">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><button style={buttonStyles.secondary} onClick={() => onNavigate('tenant-users')}><Users size={13} /> Team members</button><button style={buttonStyles.secondary} onClick={() => onNavigate('tenant-modules')}><Boxes size={13} /> Tool access</button></div>
      </section>

      <details style={{ ...cardStyle, marginBottom: space.lg }}><summary style={{ cursor: 'pointer', minHeight: 44 }}>Advanced connection settings</summary>
      <p style={{ color: semantic.textMuted }}>For the administrator connecting a supported service. Saving these settings does not activate a vendor account or prove delivery.</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,340px),1fr))', gap: 14, marginBottom: space.lg }}>
      ...
      <OperationsTable scannerReady={overview.attachments?.scanner?.configured === true} operations={operations} modules={modules} busy={busy} run={run} tenantId={tenantId} />
  ...
  return <section style={{ ...cardStyle, marginTop: space.lg }} data-testid="cross-module-provenance">
    ...
    {runs.length ? runs.map((item: any) => <article key={item.id} data-testid={`fabric-run-${item.id}`} style={{ borderTop: `1px solid ${semantic.border}`, padding: '10px 0', display: 'grid', gap: 7 }}>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', fontSize: 12 }}>
        ...
        <span style={{ color: semantic.textMuted }}>Attempts {item.attempt_count ?? 0}/{item.max_attempts ?? 0} · repeated requests {item.replay_count ?? 0}</span>
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', fontSize: 12 }}>
        ...
        <details style={{ color: semantic.textMuted }}><summary>Support details</summary><code>{item.workflow_key}{item.last_error_code ? ` · ${item.last_error_code}` : ''}</code></details>
...
function ProviderSetup({ busy, providers, run, tenantId }: any) {
  ...
  return <section style={cardStyle} data-testid="provider-setup">
    ...
    <div style={{ display: 'grid', gap: 9 }}>
      {field('Connection name', <input style={inputStyle} value={providerKey} onChange={e => setProviderKey(e.target.value)} />)}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {field('Service type', <select style={inputStyle} value={kind} onChange={e => setKind(e.target.value)}>{[['email','Email'],['sms','Text messages'],['ai','AI assistance'],['storage','File storage'],['oauth','Account connection'],['webhook','Update notifications']].map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select>)}
        {field('Availability', <select style={inputStyle} value={mode} onChange={e => setMode(e.target.value)}>{[['disabled','Off'],['test','Test only'],['live','Live settings']].map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select>)}
      </div>
      {field('Service access key or secure reference', <input type="password" autoComplete="new-password" style={inputStyle} value={secretReference} onChange={e => setSecretReference(e.target.value)} placeholder="Provided by your service administrator" />)}
    ...
    <div style={{ marginTop: 12, display: 'grid', gap: 6 }}>{providers.map((p: any) => <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 12 }}><span>{p.providerKey} · {p.kind}{p.hasSecretReference ? ' · encrypted' : ''}</span><StateBadge value={p.state === 'ready' ? 'configured' : p.state} /></div>)}</div>
  </section>;
}
...
function WebhookSetup({ busy, endpoints, modules, defaultModule, run, tenantId }: any) {
  ...
  return <section style={cardStyle} data-testid="webhook-management">
    ...
    <div style={{ marginTop: 10, fontSize: 12, color: semantic.textMuted }}>{endpoints.length ? endpoints.map((e: any) => <div key={e.id}>{e.name} · {e.endpoint_url} · <StateBadge value={e.enabled ? 'active' : 'disabled'} /></div>) : 'No update destinations are connected.'}</div>
  </section>;
}
...
function TokenSetup({ busy, tokens, run, tenantId, oneTimeToken, setOneTimeToken }: any) {
  ...
  return <section style={cardStyle} data-testid="api-token-management">
    <h2 style={{ margin: '0 0 10px', fontSize: 16 }}><KeyRound size={16} /> Connection access keys</h2>
    <div style={{ display: 'grid', gap: 9 }}>{field('Connected service name', <input style={inputStyle} value={identityName} onChange={e => setIdentityName(e.target.value)} />)}{field('Access key name', <input style={inputStyle} value={tokenName} onChange={e => setTokenName(e.target.value)} />)}<button style={buttonStyles.primary} disabled={busy === 'token'} onClick={create}>Create limited access key</button></div>
...
function OperationsTable({ operations, modules, busy, run, tenantId, scannerReady }: any) {
  ...
  return <section style={cardStyle} data-testid="shared-service-operations"><h2 style={{ margin: '0 0 12px', fontSize: 16 }}>Shared-service health and recovery</h2>
    ...
    <OperationGroup title="File safety checks" rows={attachments} render={(row: any) => <><span>{row.original_name} · {row.size_bytes} bytes</span><StateBadge value={row.scan_status} />{scannerReady && ['unavailable', 'error'].includes(row.scan_status) && <button style={buttonStyles.secondary} disabled={busy === `rescan-${row.id}`} onClick={() => run(`rescan-${row.id}`, async () => { const moduleSlug = modules.find((m: any) => m.id === row.module_id)?.slug; if (!moduleSlug) throw new Error('The owning app is unavailable'); await sharedPlatformApi.rescanAttachment(tenantId, row.id, moduleSlug); })}>Check file again</button>}</>} />
    <OperationGroup title="Exports" rows={exports} render={(row: any) => <><span>{row.export_type} · {row.format}</span><StateBadge value={row.status} />{row.status === 'completed' && row.result_attachment_id && <button style={buttonStyles.secondary} onClick={() => run(`download-${row.id}`, async () => { const moduleSlug = modules.find((m: any) => m.id === row.module_id)?.slug; if (!moduleSlug) throw new Error('Module mapping is unavailable'); const result: any = await sharedPlatformApi.createDownloadGrant(tenantId, row.result_attachment_id, moduleSlug); window.location.assign(`/api/shared-downloads/${encodeURIComponent(result.grant.token)}`); })}>Download file</button>}</>} />
