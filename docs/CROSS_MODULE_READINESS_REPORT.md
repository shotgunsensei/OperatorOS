...
| TradeFlowKit | Source/local state 4 candidate: lead conversion and replay-safe shared-outbox email/consent-gated SMS; full customer → job/work order → task single-record CRUD/deep links; shared Directory customers and bounded CSV import; tenant-scoped global search; bounded archived customer/job/invoice restore; numbered jobs/tasks/dependencies; Workflow Studio; direct and quote-derived invoices; versioned multi-line quote/invoice editing; history-safe archive; idempotent quote-to-job/invoice; payments; portal/documents; settings; analytics; CSV export | Server guards, server-owned lead destinations, SMS consent/opt-out, exact replay/body-drift rejection, optimistic versions, dependency-ordered archive/restore, bounded escaped search, persistence/restart, viewer denial, safe projections, Directory preservation, and cross-tenant tests pass | Pass | Fresh lead-messaging/retention/search adjacent PostgreSQL regression: 23/23; focused lead messaging: 9/9; retention static/routing: 4/4; prior clean aggregate: 866 pass, 0 fail, 6 HTTP-only skips | Production build and readiness-gated 29-step compiled runtime pass locally; deployed target is an older revision | Exact-host lead email plus archive/restore workflow passes 1/1 in 20.7 seconds with PKCE return, persisted shared-outbox destination, customer/job/task CRUD, canonical `/trash`, dependency-ordered restore, and preserved task archive; global-search and customer-import flows separately pass 1/1. No provider delivery or deployed workflow/cutover run | **Not production-ready** |
...
111 active items, 53 shared replacements, 49 explicit gaps, zero unclassified,
...
data apply/cutover, rollback rehearsal, and all 49 remaining ledger gaps keep
