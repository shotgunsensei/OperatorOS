...
import { planTechDeckImport } from '../lib/techdeck-import.js';
...
async function main() {
  ...
  const plan = planTechDeckImport(raw);
  process.stdout.write(`${JSON.stringify(plan, null, 2)}\n`);
  if (!plan.readyToApply) process.exitCode = 2;
}

main().catch(error => {
  console.error(`[techdeck-import] ${error instanceof Error ? error.message : error}`);
  process.exitCode = 1;
});
