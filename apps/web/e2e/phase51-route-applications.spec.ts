...
const products: Product[] = [
  ...
  { slug:'ninja-launch-kit',host:'https://ninjalaunchkit.operatoros.net',shell:'launchkit-module-shell',entry:'/dashboard',routes:['/dashboard','/projects','/templates','/brief','/deliverables','/review','/exports','/settings'],axe:['/dashboard','/templates','/review'] },
  { slug:'ninjamation',host:'https://ninjamation.operatoros.net',shell:'ninjamation-module-shell',entry:'/library',routes:['/dashboard','/library','/sources','/generate','/review','/runs','/versions','/settings'],axe:['/dashboard','/library','/settings'] },
  { slug:'ninja-pool-hall',host:'https://ninja-pool-hall.operatoros.net',shell:'ninja-pool-hall-module-shell',routes:['/','/practice','/cpu','/local','/online','/history','/profile','/settings'],axe:['/','/practice','/settings'] },
];
...
for (const product of products) test(`${product.slug} owns stable major routes, history, responsive layout, and accessibility`,async({page})=>{
  ...
  const evidence=resolve(process.cwd(),'../../docs/phase-51/evidence');await mkdir(evidence,{recursive:true});
