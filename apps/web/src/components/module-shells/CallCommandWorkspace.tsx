...
export default function CallCommandWorkspace({ view, recordId, hrefFor = path => path }: { view: CallCommandRouteArea; recordId?: string; hrefFor?: (path: string) => string }) {
  ...
  return <section data-testid={`callcommand-${view}-route`} data-callcommand-view={view} style={{ minWidth: 0, color: semantic.text }}>
      ...
      {['organizations', 'providers', 'compliance', 'settings'].includes(view) && <CallCommandMspWorkspace view={view} />}
