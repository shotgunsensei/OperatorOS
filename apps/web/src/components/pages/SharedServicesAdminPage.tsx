  ...
  return (
    <div style={{ padding: space.xxl, maxWidth: 1180, margin: '0 auto', color: semantic.text }} data-testid="page-shared-services-admin">
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 12, marginBottom: space.lg }} aria-label="Shared service health">
        {[
          ['Worker', overview?.worker?.started ? 'ready' : 'blocked', overview?.worker?.lastErrorCode || 'Persistent leases and retries'],
          ['Secret vault', overview?.secretVault?.configured ? 'ready' : overview?.secretVault?.mode === 'test' ? 'test' : 'blocked', overview?.secretVault?.keyVersion || overview?.secretVault?.reasonCode],
          ['Quarantine', String(overview?.counts?.quarantined_attachments ?? 0), 'attachments awaiting or failing scan'],
          ['Dead letters', String(Number(overview?.queues?.jobs_dead_letter || 0) + Number(overview?.queues?.outbound_webhooks_dead_letter || 0) + Number(overview?.queues?.outbox_dead_letter || 0)), 'recoverable with audited retry'],
      ...
      <section style={{ ...cardStyle, marginBottom: space.lg }} data-testid="shared-team-access-links">
        <h2 style={{ margin: '0 0 8px', fontSize: 16 }}><Users size={16} /> Team and module access</h2>
        <p style={{ color: semantic.textMuted, fontSize: 13 }}>Membership roles and per-module access remain OperatorOS authority. Manage them through the existing audited organization surfaces.</p>
...
function ProviderSetup({ busy, providers, run, tenantId }: any) {
  ...
  return <section style={cardStyle} data-testid="provider-setup">
    <h2 style={{ margin: '0 0 10px', fontSize: 16 }}><ShieldCheck size={16} /> Provider setup and health</h2>
    <div style={{ display: 'grid', gap: 9 }}>
      {field('Provider key', <input style={inputStyle} value={providerKey} onChange={e => setProviderKey(e.target.value)} />)}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {field('Kind', <select style={inputStyle} value={kind} onChange={e => setKind(e.target.value)}>{['email','sms','ai','storage','oauth','webhook'].map(v => <option key={v}>{v}</option>)}</select>)}
        {field('Mode', <select style={inputStyle} value={mode} onChange={e => setMode(e.target.value)}>{['disabled','test','live'].map(v => <option key={v}>{v}</option>)}</select>)}
      </div>
      {field('Encrypted credential reference (write-only)', <input type="password" autoComplete="new-password" style={inputStyle} value={secretReference} onChange={e => setSecretReference(e.target.value)} placeholder="vault reference or provider credential" />)}
      <label style={{ color: semantic.textMuted, fontSize: 12 }}><input type="checkbox" checked={callbackReady} onChange={e => setCallbackReady(e.target.checked)} /> Required callback verified</label>
      <button style={buttonStyles.primary} disabled={busy === 'provider'} onClick={() => run('provider', () => sharedPlatformApi.saveProvider(tenantId, providerKey, { kind, mode, secretReference: secretReference || undefined, callbackReady }))}><Send size={13} /> Save provider</button>
    </div>
...
function WebhookSetup({ busy, endpoints, modules, defaultModule, run, tenantId }: any) {
  ...
  return <section style={cardStyle} data-testid="webhook-management">
    <h2 style={{ margin: '0 0 10px', fontSize: 16 }}><Link2 size={16} /> Outbound webhooks</h2>
    <div style={{ display: 'grid', gap: 9 }}>
      {field('Module', <select style={inputStyle} value={moduleSlug} onChange={e => setModuleSlug(e.target.value)}>{modules.map((m: any) => <option key={m.slug} value={m.slug}>{m.name || m.slug}</option>)}</select>)}
      {field('Name', <input style={inputStyle} value={name} onChange={e => setName(e.target.value)} />)}
      {field('Public HTTPS endpoint', <input style={inputStyle} value={endpointUrl} onChange={e => setEndpointUrl(e.target.value)} placeholder="https://hooks.example.com/operatoros" />)}
      {field('HMAC signing secret (write-only)', <input type="password" style={inputStyle} value={signingSecret} onChange={e => setSigningSecret(e.target.value)} />)}
      {field('Event types (comma separated)', <input style={inputStyle} value={events} onChange={e => setEvents(e.target.value)} />)}
      <button style={buttonStyles.primary} disabled={busy === 'webhook'} onClick={() => run('webhook', () => sharedPlatformApi.createWebhookEndpoint(tenantId, { moduleSlug, name, endpointUrl, signingSecret, eventTypes: events.split(',').map((v: string) => v.trim()) }))}>Create endpoint</button>
    </div>
...
function ExportSetup({ busy, defaultModule, run, tenantId }: any) {
  ...
  return <section style={cardStyle} data-testid="export-management"><h2 style={{ margin: '0 0 10px', fontSize: 16 }}><Activity size={16} /> Usage, credits, and exports</h2><p style={{ color: semantic.textMuted, fontSize: 12 }}>Generate a durable, asynchronous control-plane history export. Retrieval is available only after generation and attachment scan.</p>{field('Format', <select style={inputStyle} value={format} onChange={e => setFormat(e.target.value)}><option>json</option><option>csv</option></select>)}<button style={{ ...buttonStyles.primary, marginTop: 9 }} disabled={!defaultModule || busy === 'export'} onClick={() => run('export', () => sharedPlatformApi.requestExport(tenantId, { moduleSlug: defaultModule, exportType: 'control-plane-history', format }, `ui-export:${Date.now()}`))}>Request export</button></section>;
}
...
function OperationsTable({ operations, modules, busy, run, tenantId }: any) {
  ...
  return <section style={cardStyle} data-testid="shared-service-operations"><h2 style={{ margin: '0 0 12px', fontSize: 16 }}>Worker health, recovery, quarantine, and history</h2>
    <OperationGroup title="Jobs and dead letters" rows={jobs} render={(row: any) => <><span>{row.handler_key}</span><StateBadge value={row.status} />{row.status === 'dead_letter' && <button disabled={busy === `job-${row.id}`} style={buttonStyles.secondary} onClick={() => run(`job-${row.id}`, () => sharedPlatformApi.retryDeadLetter(tenantId, 'job', row.id))}>Retry</button>}</>} />
    <OperationGroup title="Webhook deliveries" rows={webhooks} render={(row: any) => <><span>{row.endpoint_name} · {row.event_type}</span><StateBadge value={row.status} />{row.status === 'dead_letter' && <button style={buttonStyles.secondary} onClick={() => run(`webhook-${row.id}`, () => sharedPlatformApi.retryDeadLetter(tenantId, 'webhook', row.id))}>Retry</button>}</>} />
    <OperationGroup title="Attachment quarantine and scan state" rows={attachments} render={(row: any) => <><span>{row.original_name} · {row.size_bytes} bytes</span><StateBadge value={row.scan_status} /></>} />
