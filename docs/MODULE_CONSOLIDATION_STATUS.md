...
| TorqueShed | `torqueshed` | `torqueshed.operatoros.net` | free | Dirty read-only `C:\Dev\TorqueShed-Codex`: local `68da4548f665`, committed reference `508b384b6f66a1eacd3d4cd8d9c5edd4bf47fe75`; immutable snapshot remains `c33ade5...` | Phase 8 source candidate: Phase 7 automotive foundation plus server-context Torque Assist, strict evidence/safety-ranked results, shared provider limits/circuit/retry, OperatorOS-owned token packages/Stripe checkout/signed webhooks/refunds, append-only computed-balance ledger, reconciliation, metering, history and native UI. Typecheck/build/preflight pass. State remains 3 because Docker daemon failure blocks clean DB/payment/ledger/concurrency/workflow/runtime/browser evidence; marketplace/community remain Phase 9 |
...
rate/circuit and append-only ledger tables. Its apply is unverified because
