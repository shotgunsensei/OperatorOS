...
| TradeFlowKit | Source/local state 4 candidate: lead conversion; shared Directory customers and bounded CSV import; numbered jobs/tasks/dependencies; Workflow Studio; direct and quote-derived invoices; versioned multi-line quote/invoice editing; history-safe archive; idempotent quote-to-job/invoice; payments; portal/documents; messaging; settings; analytics; CSV export | Server guards, versions, exact-key import replay/body-drift rejection, persistence/restart, viewer denial, normalized duplicate/item reconciliation, Directory mapping, and cross-tenant tests pass | Pass | Clean aggregate: 866 pass, 0 fail, 6 HTTP-only skips | Compiled 29-step shared runtime, web root, health, readiness, and public route pass locally; deployed target not run | Exact-host customer-import flow passes 1/1 with PKCE return, persisted Directory reconciliation, invalid-row feedback, refresh, duplicate re-import, return, and mobile checks; prior revenue mutation flow also passes locally. Deployed workflow/cutover not run | **Not production-ready** |
