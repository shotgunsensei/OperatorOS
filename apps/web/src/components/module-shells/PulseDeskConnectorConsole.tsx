...
export default function PulseDeskConnectorConsole({ mode }: { mode: 'inbound' | 'integrations' }) {
  ...
  return <section id="pulsedesk-connectors" className="pdc" data-testid="pulsedesk-connector-console" data-connector-view={mode}>
    <header><div><span>{mode === 'inbound' ? 'Inbound communication' : 'Secure intake automation'}</span><h2>{mode === 'inbound' ? 'Operational mailbox delivery' : 'Email-to-request connectors'}</h2><p>{mode === 'inbound' ? 'Review organization-scoped mailbox routes, provider state, and bounded intake actions.' : 'Configure SendGrid, IMAP, Google Workspace, and Microsoft 365 routing with encrypted credential references, replay protection, health, and bounded worker retries.'}</p></div><strong>Operations only · PHI minimized</strong></header>
    {notice && <p role="status" className="pdc-notice">{notice}</p>}
    {mode === 'integrations' && <form onSubmit={event => { event.preventDefault(); const data=new FormData(event.currentTarget); void run('Connector setup',()=>moduleShellApi.pulsedesk.createConnector({provider:data.get('provider'),label:data.get('label'),mailboxAddress:data.get('mailboxAddress'),mode:data.get('mode'),secretReference:data.get('secretReference'),callbackReady:data.get('callbackReady')==='on'})); event.currentTarget.reset(); }}>
