...
type Tab = 'dashboard' | 'tickets' | 'operations' | 'knowledge' | 'admin';
...
export default function PulseDeskServiceDeskWorkspace({ tenantKey, canManageModule }: { tenantKey: string; canManageModule: boolean }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [dashboardData, ticketData, configData, assetData, supplyData, facilityData, knowledgeData, viewData, preferenceData, assigneeData, clientData, siteData, contactData, vendorData] = await Promise.all([
        moduleShellApi.pulsedesk.getServiceDeskDashboard(),
        moduleShellApi.pulsedesk.listServiceTickets(query),
        moduleShellApi.pulsedesk.getServiceConfiguration(),
        moduleShellApi.pulsedesk.listServiceAssets(),
        moduleShellApi.pulsedesk.listSupplyRequests(),
        moduleShellApi.pulsedesk.listFacilityRequests(),
        moduleShellApi.pulsedesk.listKnowledge(),
        moduleShellApi.pulsedesk.listSavedViews(),
        moduleShellApi.pulsedesk.getNotificationPreferences(),
        moduleShellApi.pulsedesk.listAssignees().catch(() => ({ assignees: [], capabilities: { canManageWorkflow: false } })),
        moduleShellApi.pulsedesk.listServiceClients(),
        moduleShellApi.pulsedesk.listServiceFacilities(),
        moduleShellApi.pulsedesk.listServiceContacts(),
        moduleShellApi.pulsedesk.listServiceClients('vendor'),
      ]);
      setDashboard(dashboardData); setTickets(ticketData.tickets); setTicketTotal(ticketData.pagination.total);
      setConfiguration(configData); setAssets(assetData.assets); setSupplyRequests(supplyData.supplyRequests);
      setFacilityRequests(facilityData.facilityRequests); setKnowledge(knowledgeData.articles);
      setSavedViews(viewData.savedViews); setPreferences(preferenceData); setAssignees(assigneeData.assignees);
      setClients(clientData.organizations.filter(row => row.type !== 'vendor'));
      setSites(siteData.sites); setContacts(contactData.contacts); setVendors(vendorData.organizations);
      setSelected(current => new Set([...current].filter(id => ticketData.tickets.some(ticket => ticket.id === id))));
      if (detail) {
        const fresh = ticketData.tickets.find(ticket => ticket.id === detail.ticket.id);
        if (!fresh) { setDetail(null); setAttachments([]); }
      }
  ...
  useEffect(() => {
    ...
    if (/\/(tickets|requests)(\/|$)/.test(path)) {
      setTab('tickets');
      if (ticketMatch?.[1]) void loadTicket(ticketMatch[1]);
    }
    else if (/\/(assets|supply-requests|facility-requests)(\/|$)/.test(path)) setTab('operations');
    else if (/\/knowledge(\/|$)/.test(path)) setTab('knowledge');
    else if (/\/(service-desk\/admin|settings)(\/|$)/.test(path)) setTab('admin');
  }, [loadTicket]);
  ...
  if (loading && !dashboard) return <div className="pds-loading" data-testid="pulsedesk-service-desk-loading"><RefreshCw className="pds-spin" size={18} /> Loading persisted service-desk data…<style>{css}</style></div>;
    ...
    <section className="pds" data-testid="pulsedesk-service-desk-workspace">
      ...
      <div className="pds-toolbar">
        <div className="pds-tabs" role="tablist">
          {(['dashboard', 'tickets', 'operations', 'knowledge', 'admin'] as Tab[]).map(value => (
            <button key={value} type="button" role="tab" aria-selected={tab === value} onClick={() => setTab(value)} data-testid={`pulsedesk-tab-${value}`}>{value}</button>
          ))}
        </div>
      ...
      {tab === 'dashboard' && dashboard && <Dashboard dashboard={dashboard} tickets={tickets} onOpen={id => { setTab('tickets'); void loadTicket(id); }} />}

      {tab === 'tickets' && <div className="pds-ticket-layout">
        <div className="pds-stack">
          <form className="pds-card pds-form" onSubmit={createTicket} data-testid="pulsedesk-service-ticket-create">
            <Heading icon={Plus} title="New operational ticket" subtitle="The server assigns the tenant-safe PD number, lifecycle, audit, and SLA targets." />
            <input name="summary" minLength={5} maxLength={160} required placeholder="Short operational summary" />
            <textarea name="description" maxLength={10000} placeholder="Operational context only (optional)" />
            <div className="pds-grid-3">
              <select name="ticketTypeKey" defaultValue="service_request"><option value="service_request">Service request</option><option value="incident">Incident</option><option value="problem">Problem</option><option value="maintenance">Maintenance</option><option value="supply">Supply</option><option value="facility">Facility</option></select>
              <select name="category" defaultValue="other">{(configuration?.defaults.categories ?? ['other']).map(value => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select>
              <select name="priority" defaultValue="normal">{(configuration?.defaults.priorities ?? ['normal']).map(value => <option key={value} value={value}>{value}</option>)}</select>
            </div>
            <div className="pds-grid-3"><select name="directoryOrganizationId" defaultValue=""><option value="">No service client</option>{clients.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="directorySiteId" defaultValue=""><option value="">No facility</option>{sites.map(row => <option key={row.id} value={row.id}>{row.organization?.name ? `${row.organization.name} · ` : ''}{row.name}</option>)}</select><select name="requesterContactId" defaultValue=""><option value="">No requester contact</option>{contacts.map(row => <option key={row.id} value={row.id}>{row.firstName} {row.lastName}</option>)}</select></div>
            <div className="pds-grid-3"><input name="locationLabel" maxLength={120} placeholder="Operational location" /><select name="departmentId" defaultValue=""><option value="">No department</option>{configuration?.departments.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="assetId" defaultValue=""><option value="">No equipment</option>{assets.map(row => <option key={String(row.id)} value={String(row.id)}>{String(row.assetTag)} · {String(row.name)}</option>)}</select></div>
            <div className="pds-grid-3"><select name="queueId" defaultValue=""><option value="">Unqueued</option>{configuration?.queues.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="teamId" defaultValue=""><option value="">No team</option>{configuration?.teams.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="slaPolicyId" defaultValue=""><option value="">Default/no SLA</option>{configuration?.slaPolicies.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select></div>
          ...
          <section className="pds-card">
            <Heading icon={Inbox} title={`Ticket queue (${ticketTotal})`} subtitle="Server-side search, filters, sorting, pagination, saved views, and safe bulk actions." />
            <div className="pds-filters"><Search size={15} /><input value={filters.search} onChange={event => setFilters(current => ({ ...current, search: event.target.value }))} placeholder="Search summary or location" /><select value={filters.status} onChange={event => setFilters(current => ({ ...current, status: event.target.value }))}><option value="">All statuses</option>{configuration?.defaults.statuses.map(value => <option key={value}>{value}</option>)}</select><select value={filters.priority} onChange={event => setFilters(current => ({ ...current, priority: event.target.value }))}><option value="">All priorities</option>{configuration?.defaults.priorities.map(value => <option key={value}>{value}</option>)}</select></div>
            ...
            <form className="pds-inline" onSubmit={createSavedView}><input name="name" required maxLength={100} placeholder="Save current filters as…" /><label className="pds-check"><input type="checkbox" name="shared" disabled={!canManageModule} /> Shared</label><button>Save view</button></form>
      ...
      {tab === 'operations' && <div className="pds-ops-grid">
        <OperationCard icon={Activity} title="Operational equipment" count={assets.length} form={<form className="pds-form" onSubmit={createAsset}><input name="assetTag" required placeholder="Asset tag" /><input name="name" required placeholder="Equipment name" /><input name="equipmentType" required defaultValue="operational_equipment" /><select name="status"><option>active</option><option>maintenance</option><option>out_of_service</option><option>retired</option></select><select name="directoryOrganizationId" defaultValue=""><option value="">No service client</option>{clients.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="directorySiteId" defaultValue=""><option value="">No facility</option>{sites.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="departmentId" defaultValue=""><option value="">No department</option>{configuration?.departments.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><button><Plus size={13} /> Add equipment</button></form>} rows={assets.map(row => ({ id: String(row.id), title: `${row.assetTag} · ${row.name}`, meta: `${row.equipmentType} · ${row.status}` }))} />
        <OperationCard icon={PackageCheck} title="Supply requests" count={supplyRequests.length} form={<form className="pds-form" onSubmit={createSupply}><input name="itemName" required placeholder="Supply item" /><input name="quantity" type="number" min={1} max={100000} defaultValue={1} /><select name="urgency"><option>normal</option><option>high</option><option>critical</option><option>low</option></select><select name="departmentId" defaultValue=""><option value="">No department</option>{configuration?.departments.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="ticketId" defaultValue=""><option value="">No linked ticket</option>{tickets.map(row => <option key={row.id} value={row.id}>{row.humanId} · {row.summary}</option>)}</select><button><Plus size={13} /> Request supply</button></form>} rows={supplyRequests.map(row => ({ id: String(row.id), title: `${row.quantity} × ${row.itemName}`, meta: `${row.status} · ${row.urgency}` }))} />
        <OperationCard icon={Wrench} title="Facility requests" count={facilityRequests.length} form={<form className="pds-form" onSubmit={createFacility}><input name="title" required placeholder="Facility need" /><input name="requestType" required defaultValue="maintenance" /><input name="locationLabel" placeholder="Location" /><select name="priority"><option>normal</option><option>high</option><option>critical</option><option>low</option></select><select name="directorySiteId" defaultValue=""><option value="">No facility</option>{sites.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="departmentId" defaultValue=""><option value="">No department</option>{configuration?.departments.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="ticketId" defaultValue=""><option value="">No linked ticket</option>{tickets.map(row => <option key={row.id} value={row.id}>{row.humanId} · {row.summary}</option>)}</select><button><Plus size={13} /> Create facility request</button></form>} rows={facilityRequests.map(row => ({ id: String(row.id), title: String(row.title), meta: `${row.requestType} · ${row.status}` }))} />
      </div>}

      {tab === 'knowledge' && <div className="pds-two"><section className="pds-card"><Heading icon={BookOpen} title="Operational knowledge" subtitle="Published requester guidance and internal service procedures are plain text and PHI-minimized." />{knowledge.length ? knowledge.map(article => <article className="pds-row" key={String(article.id)}><strong>{String(article.title)}</strong><small>{String(article.status)} · {String(article.visibility)}</small><p>{String(article.summary ?? '')}</p></article>) : <Empty text="No visible knowledge articles." />}</section>{canManageModule && <form className="pds-card pds-form" onSubmit={createKnowledge}><Heading icon={Plus} title="Author knowledge" subtitle={PHI_WARNING} /><input name="title" required placeholder="Article title" /><input name="summary" placeholder="Summary" /><textarea name="body" required placeholder="Operational procedure in plain text" /><select name="visibility"><option value="internal">Internal</option><option value="requester">Requester-visible</option></select><select name="status"><option value="draft">Draft</option><option value="published">Published</option></select><button>Save article</button></form>}</div>}

      {tab === 'admin' && <div className="pds-two">
        <section className="pds-card"><Heading icon={Settings} title="Workflow configuration" subtitle="Queues, teams, fixed lifecycle labels, SLA policies, and notification preferences are persisted." /><ConfigRows configuration={configuration} />
        ...
        {canManageModule ? <section className="pds-stack"><form className="pds-card pds-form" onSubmit={createQueue}><Heading icon={UsersRound} title="Add queue" subtitle="Queues route operational work; teams can be linked through the API." /><input name="name" required placeholder="Queue name" /><textarea name="description" placeholder="Queue purpose" /><button>Add queue</button></form><form className="pds-card pds-form" onSubmit={createSla}><Heading icon={Clock3} title="Add SLA policy" subtitle="Targets use elapsed minutes; calendar-aware business hours are not claimed." /><input name="name" required placeholder="Policy name" /><div className="pds-grid-3"><input name="responseMinutes" type="number" min={1} defaultValue={60} /><input name="resolutionMinutes" type="number" min={1} defaultValue={480} /><input name="atRiskPercent" type="number" min={1} max={99} defaultValue={80} /></div><label className="pds-check"><input type="checkbox" name="defaultPolicy" /> Tenant default</label><button>Add policy</button></form></section> : <section className="pds-card"><Empty text="Manager access is required for administrative mutations." /></section>}
      </div>}
    </section>
  );
}
...
function Dashboard({ dashboard, tickets, onOpen }: { dashboard: PulseDeskServiceDashboard; tickets: PulseDeskServiceTicket[]; onOpen: (id: string) => void }) {
  ...
  return <div className="pds-stack"><div className="pds-metrics">{metrics.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div><section className="pds-card"><Heading icon={Activity} title="Current operational work" subtitle={`Real persisted aggregates generated ${formatDate(dashboard.generatedAt)}.`} />{tickets.slice(0, 8).map(ticket => <button className="pds-row-button" key={ticket.id} onClick={() => onOpen(ticket.id)}><span><strong>{ticket.humanId}</strong> {ticket.summary}</span><small>{ticket.status} · {ticket.priority} · SLA {ticket.sla.state}</small></button>)}</section></div>;
}
...
function TicketDetail({ detail, busy, attachments, configuration, assignees, vendors, onUpdate, onReply, onTime, onAssign, onTransition, onEvaluate, onUpload, onVendor }: {
  ...
  if (!detail) return <aside className="pds-card pds-detail"><Empty text="Select a ticket to open its persisted workspace." /></aside>;
  const { ticket } = detail;
  return <aside className="pds-card pds-detail" data-testid="pulsedesk-ticket-workspace"><Heading icon={Inbox} title={`${ticket.humanId} · ${ticket.summary}`} subtitle={`${ticket.status.replaceAll('_', ' ')} · ${ticket.priority} · SLA ${ticket.sla.state.replaceAll('_', ' ')}`} /><p className="pds-description">{ticket.description || 'No additional operational description.'}</p><div className="pds-chips"><span>Created {formatDate(ticket.createdAt)}</span><span>Response due {formatDate(ticket.responseDueAt)}</span><span>Resolution due {formatDate(ticket.resolutionDueAt)}</span><span>Version {ticket.version}</span></div>
    {detail.capabilities.canViewInternal && <form key={`${ticket.id}-${ticket.version}`} className="pds-form" onSubmit={onUpdate}><h4>Edit operational ticket</h4><input name="summary" required minLength={5} maxLength={160} defaultValue={ticket.summary} /><textarea name="description" maxLength={10000} defaultValue={ticket.description} /><div className="pds-grid-3"><select name="priority" defaultValue={ticket.priority}>{configuration?.defaults.priorities.map(value => <option key={value}>{value}</option>)}</select><select name="category" defaultValue={ticket.category}>{configuration?.defaults.categories.map(value => <option key={value}>{value}</option>)}</select><select name="ticketTypeKey" defaultValue={ticket.ticketTypeKey}>{configuration?.defaults.types.map(value => <option key={value}>{value}</option>)}</select></div><input name="locationLabel" maxLength={120} defaultValue={ticket.locationLabel ?? ''} placeholder="Operational location" /><label className="pds-ack">{PHI_WARNING}</label><button disabled={Boolean(busy)}>Save ticket changes</button></form>}
    {detail.capabilities.canManage && <form className="pds-inline" onSubmit={onAssign}><select name="assignedToUserId" defaultValue={ticket.assignedToUserId ?? ''}><option value="">Unassigned</option>{assignees.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="queueId" defaultValue={ticket.queueId ?? ''}><option value="">No queue</option>{configuration?.queues.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><select name="teamId" defaultValue={ticket.teamId ?? ''}><option value="">No team</option>{configuration?.teams.filter(row => row.active).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><button disabled={Boolean(busy)}>Assign</button></form>}
    ...
    <form className="pds-form" onSubmit={event => onReply(event, 'requester')}><textarea name="body" required placeholder="Requester-visible operational reply" /><label className="pds-ack">{PHI_WARNING}</label><button>Add requester-visible reply</button></form>
    {detail.capabilities.canViewInternal && <><form className="pds-form" onSubmit={event => onReply(event, 'internal')}><textarea name="body" required placeholder="Internal service-team note" /><label className="pds-ack">Internal notes are never returned to requester/viewer roles. {PHI_WARNING}</label><button>Add internal note</button></form><form className="pds-inline" onSubmit={onTime}><input name="minutes" type="number" min={1} max={1440} required placeholder="Minutes" /><select name="workType"><option>onsite</option><option>remote</option><option>vendor</option><option>administrative</option></select><input name="description" placeholder="Operational work summary" /><button>Log time</button></form></>}
    ...
    <section><h4>Attachments</h4><form className="pds-inline" onSubmit={onUpload}><input name="file" type="file" required /><select name="visibility"><option value="requester">Requester-visible</option>{detail.capabilities.canViewInternal && <option value="internal">Internal</option>}</select><button><FileUp size={13} /> Upload</button></form>{attachments.length ? attachments.map(file => <div className="pds-row" key={String(file.id)}><strong>{String(file.original_name ?? file.originalName ?? 'Attachment')}</strong><small>{String(file.visibility)} · {String(file.scan_status ?? file.scanStatus ?? 'scan pending')}</small></div>) : <Empty text="No attachments." />}</section>
    {detail.capabilities.canViewInternal && <form className="pds-form" onSubmit={onVendor}><h4>Vendor coordination</h4><select name="vendorOrganizationId" required defaultValue=""><option value="" disabled>Select a shared Directory vendor</option>{vendors.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><div className="pds-grid-3"><select name="status"><option>requested</option><option>acknowledged</option><option>scheduled</option><option>waiting</option><option>completed</option><option>cancelled</option></select><input name="referenceCode" placeholder="Vendor reference" /></div><button>Add vendor engagement</button></form>}
  </aside>;
}
