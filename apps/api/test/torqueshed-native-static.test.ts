...
test('native security, offline, association, and release gates are explicit', () => {
  ...
  assert.match(auth, /pending-revocations/); assert.match(auth, /public\/torqueshed\/native\/logout/); assert.match(auth, /NetInfo\.addEventListener/); assert.match(auth, /revocationFlushInFlight/); assert.match(auth, /serializeRevocationOperation/);
  ...
  assert.match(sync, /syncingRef/); assert.match(sync, /queuedScopeKeyRef/); assert.match(sync, /NATIVE_SYNC_SCOPE_CHANGED/); assert.doesNotMatch(sync, /\[online, scope\?\.tenantId, scope\?\.userId, syncing\]/);
