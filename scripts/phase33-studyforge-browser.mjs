...
const runtimeEnv = {
  ...
  APP_ENV: 'test',
  NODE_ENV: 'production',
  SESSION_SECRET: process.env.SESSION_SECRET || 'studyforge-phase33-exact-host-session-secret',
  SSO_CODE_ENCRYPTION_SECRET: process.env.SSO_CODE_ENCRYPTION_SECRET || 'studyforge-phase33-exact-host-sso-secret',
  SHARED_SECRET_ENCRYPTION_KEY: process.env.SHARED_SECRET_ENCRYPTION_KEY || '33'.repeat(32),
  TRUST_PROXY: '1',
  API_PORT: '5001',
  NEXT_INTERNAL_PORT: '5002',
  PORT: '5001',
...
try {
  api = spawnLogged(process.execPath, ['--conditions=production', 'apps/api/dist/apps/api/src/index.js'], {
    cwd: REPOSITORY_ROOT,
    env: runtimeEnv,
    logPath: join(BUILD_ROOT, 'phase33-api.log'),
  });
  ...
  web = spawnLogged(process.execPath, ['node_modules/next/dist/bin/next', 'start', '-p', '5002'], {
    ...
    logPath: join(BUILD_ROOT, 'phase33-web.log'),
  });
  ...
  gateway = spawnLogged(process.execPath, ['scripts/hotfix/exact-host-gateway.mjs'], {
    cwd: REPOSITORY_ROOT,
    env: { ...runtimeEnv, PORT: '5000' },
    logPath: join(BUILD_ROOT, 'phase33-gateway.log'),
  });
  await waitForPort(5000);

  proxy = spawnLogged(process.execPath, ['apps/web/e2e/production-host-proxy.mjs'], {
    cwd: REPOSITORY_ROOT,
    env: { ...runtimeEnv, E2E_PROXY_TARGET: 'http://127.0.0.1:5000' },
    logPath: join(BUILD_ROOT, 'phase33-exact-host-proxy.log'),
  });
  await waitForPort(443);

  exitCode = run(playwright, ['test', 'e2e/studyforge-phase33.spec.ts'], {
    cwd: webRoot,
    env: runtimeEnv,
  });
} finally {
