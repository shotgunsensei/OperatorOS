...
| 16A | Re-baseline TradeFlowKit against the restored full product and close recorded parity gaps | Phase 15 release evidence plus restored-source provenance | In progress. The executable source ledger has zero unclassified items and 8 explicit gaps after twelve real increments: Workflow Studio/team work management, persistent revenue-document mutations, bounded Directory-reconciled customer CSV import, full customer/job/task record editing, bounded tenant-scoped global search with canonical result navigation, tenant-safe archived customer/job/invoice listing plus dependency-ordered restore, replay-safe lead email/consent-gated SMS through the shared outbox, durable saved job views, versioned bounded QuickBooks/Xero accounting exports, non-destructive versioned safe-bulk operations, versioned internal lead settings/templates with transactional manually actioned follow-ups plus sanitized adapter validation/history, and bounded tenant/customer-reconciled job plus exact-cent invoice imports. ADR-0031 keeps standalone tasks, autonomous scheduling, module-owned communication providers, and unreviewed lead AI outside the accepted scope; ADR-0030 keeps automatic response and anonymous lead intake disabled; destructive bulk deletion and permanent purge remain excluded under ADR-0011/0029. The remaining gaps are three public-intake and five production business-payment items. No real standalone export, production apply, deployment, live-provider acceptance, or state-5 promotion is authorized. |
