...
export const PHASE13_MIGRATION_MANIFESTS: readonly ModuleMigrationManifest[] = Object.freeze([
  ...
  {
    slug: 'faultlinelab',
    source: { system: 'C:\\Dev\\Faultline-Lab snapshot', version: 'compiler-discovered allCases manifest', commit: '46877aae35565149ccf4f4988dd94627fc6bb92b', exportMethod: 'Deterministic read-only compilation of the pinned allCases export; no standalone database apply.' },
    targetReleaseStep: 'faultlinelab_tables',
    mappings: ['every compiler-discovered authored challenge and immutable version', 'source IDs/slugs/source hashes/content hashes', 'catalog metadata and explicit compiler repairs'],
    reconciliation: ['unique source IDs/slugs', 'content hashes', 'zero authority/billing imports'],
    conflicts: 'Reject hash drift, duplicate IDs/slugs, invalid evidence references, alias collisions, or scoring bounds; never silently exclude an authored case.',
    rollback: sharedRollback, writeFreeze: sharedFreeze,
