...
type RequestedRecordValue = TechDeckAsset | TechDeckDocument | TechDeckEvidence | TechDeckReport;
...
function requestedRecordFor(area: TechDeckOperationsArea, recordId?: string): RequestedRecord | null {
  if (!recordId) return routeRecord();
  if (['inventory', 'network', 'lifecycle'].includes(area)) return { kind: 'configuration', id: recordId };
  if (['documentation', 'runbooks'].includes(area)) return { kind: 'document', id: recordId };
  if (area === 'evidence') return { kind: 'evidence', id: recordId };
  if (area === 'reports') return { kind: 'report', id: recordId };
  return null;
}

function getRequestedRecord(record: RequestedRecord): Promise<RequestedRecordValue> {
  if (record.kind === 'configuration') return moduleShellApi.techdeck.getConfigurationItem(record.id);
  if (record.kind === 'document') return moduleShellApi.techdeck.getDocument(record.id);
  if (record.kind === 'evidence') return moduleShellApi.techdeck.getEvidence(record.id);
  return moduleShellApi.techdeck.getReport(record.id);
}

function prependUnique<T extends { id: string }>(rows: T[], requested: T | null): T[] {
  return requested && !rows.some((row) => row.id === requested.id) ? [requested, ...rows] : rows;
}
...
export default function TechDeckOperations({ tenantKey, canWrite, canApprove, area, recordId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const requested = requestedRecordFor(area, recordId);
    setRequestedRecord(requested);
    if (requested?.kind === 'report') setSelectedReportId(requested.id);
    try {
      const [workspace, directory, exact] = await Promise.all([
      ...
      if (exact.value && requested?.kind === 'configuration') {
        ...
        merged.reports = prependUnique(workspace.reports, exact.value as TechDeckReport);
      }
      setData(merged); setOrganizations(directory.organizations);
      if (exact.unavailable) {
        setError('The requested TechDeck record is not available in this organization. It may have been removed, or your role may not allow access.');
      }
  ...
  const selectedReport = useMemo(() => {
    ...
    return data.reports.find(row => row.id === selectedReportId) ?? data.reports[0];
  ...
  const requestedRecordState = useMemo(() => {
    ...
    if (requestedRecord.kind === 'configuration') {
      ...
      return { kind: 'managed asset', found: Boolean(row), label: row?.name };
    }
  ...
  async function action<T>(key: string, work: () => Promise<T>, reset?: () => void): Promise<T | null> {
    if (!canWrite) return null;
    setBusy(key); setError(null);
    try { const result = await work(); reset?.(); await load(); return result; }
  ...
  function createReport(event: React.FormEvent) {
    ...
    void action('report-create', () => moduleShellApi.techdeck.generateReport(report.name, report.reportType))
      .then(created => { if (created) setSelectedReportId(created.id); });
  }

  async function downloadReportFile(row: TechDeckReport, format: 'json' | 'csv') {
    const key = `report-download-${row.id}-${format}`;
    setBusy(key); setError(null);
    try {
      saveBlob(await moduleShellApi.techdeck.downloadReport(row.id, format), `techdeck-report-${row.id}.${format}`);
    } catch (err) { setError(message(err)); }
    finally { setBusy(null); }
  }
  ...
  function transitionDocument(row: TechDeckDocument) {
    ...
    if (!canWrite || !transition || ((transition === 'approve' || transition === 'publish') && !canApprove)) return;
  ...
  return (
    <section id="techdeck-ops" className="techdeck-panel td-workspace" data-testid="techdeck-ops-workspace" tabIndex={-1}>
      ...
      {!canWrite && <div className="td-read-only" role="status" data-testid="techdeck-operations-read-only"><ShieldCheck size={16} /><span><strong>Read-only operations workspace.</strong> You can review assets, networks, documentation, evidence, reports, and time entries. Ask an organization administrator for edit access before changing service records.</span></div>}
      ...
      {requestedRecordState && <div className="td-route-context" data-testid="techdeck-route-record-context" data-found={requestedRecordState.found}>
        {requestedRecordState.found
          ? <><CheckCircle2 size={16} /><span>Opened {requestedRecordState.kind}: <strong>{requestedRecordState.label}</strong></span></>
      ...
      {['inventory', 'network', 'lifecycle'].includes(area) && <div className="td-summary">
        ...
        <Summary label="Network records" value={networkItems.length} Icon={Network} />
      ...
      {area === 'inventory' && <Panel id="techdeck-inventory" title="Managed asset inventory" icon={<ServerCog size={17} />}>
        {canWrite && <form className="td-form td-item-form" onSubmit={createItem} data-testid="techdeck-configuration-create-form">
          <input required aria-label="Asset name" placeholder="Asset name" value={item.name} onChange={event => setItem({ ...item, name: event.target.value })} />
          <select aria-label="Asset type" value={item.type} onChange={event => setItem({ ...item, type: event.target.value as TechDeckAssetType })}>{assetTypes.map(value => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select>
          <select aria-label="Asset client" value={item.directoryOrganizationId} onChange={event => setItem({ ...item, directoryOrganizationId: event.target.value })}><option value="">Unassigned client</option>{organizations.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select>
          <input aria-label="Asset hostname" placeholder="Hostname" value={item.hostname} onChange={event => setItem({ ...item, hostname: event.target.value })} />
          <input aria-label="Asset IP address" placeholder="IP address" value={item.ipAddress} onChange={event => setItem({ ...item, ipAddress: event.target.value })} />
          <input aria-label="Asset network range" placeholder="Network range (10.0.0.0/24)" value={item.cidr} onChange={event => setItem({ ...item, cidr: event.target.value })} />
          <input aria-label="Asset VLAN" inputMode="numeric" placeholder="VLAN" value={item.vlanNumber} onChange={event => setItem({ ...item, vlanNumber: event.target.value })} />
      ...
      {area === 'network' && <Panel id="techdeck-network" title="Network and IPAM relationships" icon={<Network size={17} />}>
          {canWrite && data && data.configurationItems.length > 1 && <form className="td-form" onSubmit={createRelationship} data-testid="techdeck-relationship-create-form">
            <select required aria-label="First asset" value={relationship.sourceAssetId} onChange={event => setRelationship({ ...relationship, sourceAssetId: event.target.value })}><option value="">First asset</option>{data.configurationItems.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select>
            <select required aria-label="Related asset" value={relationship.targetAssetId} onChange={event => setRelationship({ ...relationship, targetAssetId: event.target.value })}><option value="">Related asset</option>{data.configurationItems.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select>
          ...
          <div className="td-list">{data?.relationships.map(row => <div className="td-row" key={row.id}><span>{data.configurationItems.find(item => item.id === row.sourceAssetId)?.name ?? 'Asset'} <b>{row.relationshipType.replaceAll('_', ' ')}</b> {data.configurationItems.find(item => item.id === row.targetAssetId)?.name ?? 'Asset'}</span></div>)}{!data?.relationships.length && <Empty text="No asset relationships recorded." />}</div>
      </Panel>}
      ...
      {area === 'reports' && <Panel id="techdeck-reports" title="Operations reports" icon={<BarChart3 size={17} />}>
          ...
          <div className="td-report-layout">
            <div className="td-list">{data?.reports.map(row => <button type="button" className="td-row td-report-choice" key={row.id} data-record-id={row.id} data-active={selectedReport?.id === row.id} aria-pressed={selectedReport?.id === row.id} onClick={() => setSelectedReportId(row.id)}><div><strong>{row.name}</strong><small>{row.reportType.replaceAll('_', ' ')} · created {new Date(row.createdAt).toLocaleString()}</small></div><span>View report</span></button>)}{!data?.reports.length && <Empty text="No operations reports have been created yet." />}</div>
            {selectedReport && <ReportViewer report={selectedReport} busy={busy} onDownload={downloadReportFile} />}
          </div>
      </Panel>}
...
function Breakdown({ title, values }: { title: string; values: Record<string, number> }) {
  ...
  return <section><h5>{title}</h5>{rows.length ? <ul>{rows.map(([label, value]) => <li key={label}><span>{label.replaceAll('_', ' ')}</span><strong>{value}</strong></li>)}</ul> : <p>No matching records.</p>}</section>;
}
function ReportViewer({ report, busy, onDownload }: { report: TechDeckReport; busy: string | null; onDownload: (report: TechDeckReport, format: 'json' | 'csv') => Promise<void> }) {
  const assets = report.snapshot.configurationItems ?? {};
  const tickets = report.snapshot.tickets ?? {};
  const time = report.snapshot.time ?? {};
  const health = assets.byHealth ?? {};
  const ticketStatuses = tickets.byStatus ?? {};
  const attentionItems = (health.warning ?? 0) + (health.critical ?? 0) + (health.offline ?? 0);
  const closedTickets = (ticketStatuses.resolved ?? 0) + (ticketStatuses.closed ?? 0);
  const openTickets = Math.max(0, (tickets.total ?? 0) - closedTickets);
  const billableHours = Math.round(((time.billableMinutes ?? 0) / 60) * 10) / 10;
  return <article className="td-report-viewer" data-testid="techdeck-report-viewer">
    <header><div><span>Report results</span><h4>{report.name}</h4><p>{report.reportType.replaceAll('_', ' ')} · created {new Date(report.createdAt).toLocaleString()}</p></div><div className="td-report-downloads"><button type="button" className="td-button" disabled={busy === `report-download-${report.id}-json`} onClick={() => void onDownload(report, 'json')}><Download size={14} />JSON</button><button type="button" className="td-button" disabled={busy === `report-download-${report.id}-csv`} onClick={() => void onDownload(report, 'csv')}><Download size={14} />CSV</button></div></header>
    <div className="td-report-metrics">
      <Metric label="Managed items" value={assets.total ?? 0} />
      <Metric label="Need attention" value={attentionItems} />
      <Metric label="Open tickets" value={openTickets} />
      <Metric label="Evidence records" value={report.snapshot.evidenceCount ?? 0} />
      <Metric label="Billable hours" value={billableHours} />
    </div>
    <div className="td-report-breakdowns">
      <Breakdown title="Asset health" values={health} />
      <Breakdown title="Asset types" values={assets.byType ?? {}} />
      <Breakdown title="Ticket priorities" values={tickets.byPriority ?? {}} />
      <Breakdown title="Ticket status" values={ticketStatuses} />
    </div>
    <details><summary>Technical details</summary><code>Snapshot verification: {report.sha256}</code></details>
  </article>;
}
function DocumentList({ title, rows, requestedDocumentId, canWrite, canApprove, busy, onTransition }: { title: string; rows: TechDeckDocument[]; requestedDocumentId: string | null; canWrite: boolean; canApprove: boolean; busy: string | null; onTransition: (row: TechDeckDocument) => void }) {
  return <div className="td-stack"><h4>{title}</h4><div className="td-list">{rows.map(row => {
    ...
    return <article className="td-row td-doc" key={row.id} data-record-id={row.id} data-active={requestedDocumentId === row.id}><div><strong>{row.title}</strong><small>{row.pageType.replaceAll('_', ' ')}</small><p>{row.summary || row.content.slice(0, 140)}</p></div><div className="td-actions"><Status value={row.status} />{allowed && label && <button className="td-button" disabled={busy === `doc-${row.id}`} onClick={() => onTransition(row)}>{label}</button>}</div></article>;
...
.td-button{border:1px solid rgba(56,189,248,.5);background:rgba(14,165,233,.15);color:#e5eefc;border-radius:6px;padding:8px 11px;font-weight:750;display:inline-flex;gap:7px;align-items:center;justify-content:center;cursor:pointer}.td-button:disabled{opacity:.55;cursor:not-allowed}.td-secondary{background:#101826;border-color:rgba(148,163,184,.25)}.td-error,.td-boundary,.td-route-context,.td-read-only{display:flex;gap:8px;align-items:center;border-radius:6px;padding:10px 12px;font-size:13px}.td-error{color:#fecaca;background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.35)}.td-boundary{color:#bae6fd;background:rgba(56,189,248,.08);border:1px solid rgba(56,189,248,.2)}.td-read-only{color:#cbd5e1;background:#101826;border:1px solid rgba(148,163,184,.32)}.td-read-only svg{color:#7dd3fc;flex:none}.td-route-context{color:#cbd5e1;background:#101826;border:1px solid rgba(148,163,184,.24)}.td-route-context[data-found=true]{color:#bae6fd;border-color:rgba(56,189,248,.45)}
...
.td-list{display:grid;gap:7px}.td-row{display:flex;justify-content:space-between;gap:10px;align-items:center;padding:10px;border:1px solid rgba(148,163,184,.13);border-radius:6px;background:#0d1320;font-size:13px}.td-row[data-active=true]{border-color:#38bdf8;box-shadow:0 0 0 1px rgba(56,189,248,.5);background:rgba(14,165,233,.09)}.td-row strong,.td-row small{display:block}.td-row small{margin-top:3px;color:#8fa3bd;font-size:11px}.td-row p{margin:6px 0 0;color:#8fa3bd;font-size:12px;white-space:pre-wrap}.td-doc{align-items:flex-start}.td-actions{display:grid;gap:6px;justify-items:end;flex:none}.td-status{text-transform:capitalize;border:1px solid rgba(148,163,184,.25);border-radius:999px;padding:4px 8px;font-size:10px;color:#cbd5e1}.td-status-healthy,.td-status-approved,.td-status-published{color:#86efac;border-color:rgba(34,197,94,.4)}.td-status-warning,.td-status-draft,.td-status-in_review{color:#fcd34d;border-color:rgba(245,158,11,.4)}.td-status-critical,.td-status-offline{color:#fca5a5;border-color:rgba(239,68,68,.4)}.td-empty{display:flex;gap:8px;align-items:center;color:#8fa3bd;padding:12px;font-size:13px}.td-report-layout{display:grid;grid-template-columns:minmax(230px,.7fr) minmax(0,1.5fr);gap:12px;align-items:start}.td-report-choice{width:100%;font:inherit;color:#dce9f8;text-align:left;cursor:pointer}.td-report-choice>span{font-size:11px;color:#7dd3fc;white-space:nowrap}.td-report-viewer{display:grid;gap:12px;padding:14px;border:1px solid rgba(56,189,248,.28);border-radius:7px;background:#0b1320}.td-report-viewer>header{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.td-report-viewer>header span{font-size:10px;font-weight:850;letter-spacing:.08em;text-transform:uppercase;color:#38bdf8}.td-report-viewer h4{margin:3px 0;color:#e5eefc;font-size:16px}.td-report-viewer header p{margin:0;color:#8fa3bd;font-size:11px}.td-report-downloads{display:flex;gap:6px}.td-report-metrics{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px}.td-report-metrics>div{padding:9px;border:1px solid rgba(148,163,184,.16);border-radius:6px;background:#080d16}.td-report-metrics strong,.td-report-metrics span{display:block}.td-report-metrics strong{font-size:17px;color:#e5eefc}.td-report-metrics span{margin-top:2px;font-size:10px;color:#8fa3bd}.td-report-breakdowns{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.td-report-breakdowns section{padding:9px;border:1px solid rgba(148,163,184,.13);border-radius:6px}.td-report-breakdowns h5{margin:0 0 6px;color:#cbd5e1;font-size:11px}.td-report-breakdowns ul{list-style:none;margin:0;padding:0;display:grid;gap:4px}.td-report-breakdowns li{display:flex;justify-content:space-between;gap:8px;font-size:11px;color:#8fa3bd;text-transform:capitalize}.td-report-breakdowns li strong{color:#dce9f8}.td-report-breakdowns p{margin:0;color:#64748b;font-size:11px}.td-report-viewer details{color:#8fa3bd;font-size:11px}.td-report-viewer details code{display:block;margin-top:6px;overflow-wrap:anywhere;color:#94a3b8}.td-spin{animation:tdspin 1s linear infinite}@keyframes tdspin{to{transform:rotate(360deg)}}
@media(max-width:1000px){.td-item-form{grid-template-columns:repeat(2,1fr)}.td-time-form{grid-template-columns:repeat(2,1fr)}.td-report-layout{grid-template-columns:1fr}.td-report-metrics{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:760px){.td-summary,.td-columns,.td-doc-columns,.td-form,.td-item-form,.td-doc-form,.td-time-form,.td-report-metrics,.td-report-breakdowns{grid-template-columns:1fr}.td-doc-form textarea{grid-column:auto}.td-head,.td-report-viewer>header{display:grid}.td-row{align-items:flex-start;flex-direction:column}.td-actions{justify-items:start}.td-report-downloads{flex-wrap:wrap}}
`;
