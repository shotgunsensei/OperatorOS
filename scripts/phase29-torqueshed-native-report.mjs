...
const required = [
  'apps/torqueshed-native/app.config.ts', 'apps/torqueshed-native/src/lib/auth.tsx',
  'apps/torqueshed-native/src/lib/offline-queue.ts', 'apps/api/src/lib/torqueshed-native-auth.ts',
  'apps/api/src/lib/torqueshed-native-db-init.ts', 'apps/web/src/app/.well-known/apple-app-site-association/route.ts',
  'apps/web/src/app/.well-known/assetlinks.json/route.ts', '.github/workflows/torqueshed-native.yml',
];
for (const path of required) if (!existsSync(resolve(root, path))) throw new Error(`Phase 29 required file missing: ${path}`);
const config = readFileSync(resolve(root, required[0]), 'utf8');
if (/YOUR_|PLACEHOLDER|REPLACE_ME|0000000000|AA:AA:AA/i.test(config)) throw new Error('Phase 29 release config contains a placeholder identifier');

const report = `# Phase 29 — TorqueShed Native Release Report
