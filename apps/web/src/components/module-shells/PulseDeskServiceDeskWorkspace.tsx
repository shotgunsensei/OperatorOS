      ...
      {view === 'tickets' && <div className="pds-ticket-layout">
        <div className="pds-stack">
          ...
          <section className="pds-card">
            ...
            <form className="pds-inline" onSubmit={createSavedView}><input name="name" required maxLength={100} aria-label="Saved view name" placeholder="Save current filters as…" /><label className="pds-check"><input type="checkbox" name="shared" disabled={!canManageModule} /> Shared</label><button>Save view</button></form>
...
function TicketDetail({ detail, busy, attachments, configuration, assignees, vendors, onUpdate, onReply, onTime, onAssign, onTransition, onEvaluate, onUpload, onVendor }: {
  ...
  return <aside className="pds-card pds-detail" data-testid="pulsedesk-ticket-workspace"><Heading icon={Inbox} title={`${ticket.humanId} · ${ticket.summary}`} subtitle={`${ticket.status.replaceAll('_', ' ')} · ${ticket.priority} · SLA ${ticket.sla.state.replaceAll('_', ' ')}`} /><p className="pds-description">{ticket.description || 'No additional operational description.'}</p><div className="pds-chips"><span>Created {formatDate(ticket.createdAt)}</span><span>Response due {formatDate(ticket.responseDueAt)}</span><span>Resolution due {formatDate(ticket.resolutionDueAt)}</span><span>Version {ticket.version}</span></div>
    ...
    <form className="pds-form" onSubmit={event => onReply(event, 'requester')}><textarea name="body" required aria-label="Requester-visible operational reply" placeholder="Requester-visible operational reply" /><label className="pds-ack">{PHI_WARNING}</label><button>Add requester-visible reply</button></form>
    {detail.capabilities.canViewInternal && <><form className="pds-form" onSubmit={event => onReply(event, 'internal')}><textarea name="body" required aria-label="Internal service-team note" placeholder="Internal service-team note" /><label className="pds-ack">Internal notes are never returned to requester/viewer roles. {PHI_WARNING}</label><button>Add internal note</button></form><form className="pds-inline" onSubmit={onTime}><input name="minutes" aria-label="Minutes worked" type="number" min={1} max={1440} required placeholder="Minutes" /><select name="workType" aria-label="Work type"><option>onsite</option><option>remote</option><option>vendor</option><option>administrative</option></select><input name="description" aria-label="Operational work summary" placeholder="Operational work summary" /><button>Log time</button></form></>}
